...
- Microsoft Graph の要求スコープを `Files.ReadWrite` から `Files.ReadWrite.All` に変更しました。
- 他ユーザーが共有フォルダ経由で使う場合に、ログインユーザーがアクセス可能なファイルまで読み書きできる権限を要求します。
- 固定ルートフォルダ共有リンクは `?e=...` を含まないURLに統一しました。
- 共有リンク候補から `?e=...` 付きURLを除外し、Graph APIにはクエリなし・末尾スラッシュなしURLだけを渡します。

Entra側でも Microsoft Graph の委任されたアクセス許可に `Files.ReadWrite.All` を追加してください。権限変更後は、利用者側でログアウトしてから再ログインしてください。
